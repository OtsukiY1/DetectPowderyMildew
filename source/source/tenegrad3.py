...
def tenegrad_score_map(gray, block_size):
    ...
    for col, x in enumerate(range(0, h, block_size)):
        for col, x in enumerate(range(0, w, block_size)):
            ...
            score_map[row, col] = np.mean(gx**2)
    return score_map

def process_video(video_path, output_path, block_size=64, threshold_raito=0.3, avg_window=10):
    avg_window
    ...
    for i, frame in enumerate(all_frames):
        ...
        threshold = avg_map.max() * threshold_raito
...
if __name__ == "__main__":
    process_video(
       video_path=r"c:\Users\abeke\DetectPowderyMildewSpore\test\trimVideo\38No2.mp4",
        output_path=r"c:\Users\abeke\DetectPowderyMildewSpore\test\focused\38No2_masked.mp4",
        block_size=64,     # 小さいほど細かく検出（32が次の候補）
        threshold_ratio=0.05,  # 上げると黒い領域が増える、下げると減る
        avg_window=10,
    )